from utils.func import get_data_description, get_amount_currency, get_where_from, get_last_transaction


def test_get_data_description():
    assert get_data_description({'id': 154927927, 'state': 'EXECUTED', 'date': '2019-11-19T09:22:25.899614',
                                 'operationAmount': {'amount': '30153.72', 'currency': {'name': 'руб.', 'code': 'RUB'}},
                                 'description': 'Перевод организации', 'from': 'Maestro 7810846596785568',
                                 'to': 'Счет 43241152692663622869'}) == '19.11.2019 Перевод организации'


def test_get_amount_currency():
    assert get_amount_currency({'id': 154927927, 'state': 'EXECUTED', 'date': '2019-11-19T09:22:25.899614',
                                'operationAmount': {'amount': '30153.72', 'currency': {'name': 'руб.', 'code': 'RUB'}},
                                'description': 'Перевод организации', 'from': 'Maestro 7810846596785568',
                                'to': 'Счет 43241152692663622869'}) == '30153.72 руб.'


def test_get_where_from():
    assert get_where_from({'id': 154927927, 'state': 'EXECUTED', 'date': '2019-11-19T09:22:25.899614',
                           'operationAmount': {'amount': '30153.72', 'currency': {'name': 'руб.', 'code': 'RUB'}},
                           'description': 'Перевод организации', 'from': 'Maestro 7810846596785568',
                           'to': 'Счет 43241152692663622869'}) == 'Maestro 7810 84** **** 5568 -> Счет **2869'
    assert get_where_from({'id': 801684332, 'state': 'EXECUTED', 'date': '2019-11-05T12:04:13.781725',
                           'operationAmount': {'amount': '21344.35', 'currency': {'name': 'руб.', 'code': 'RUB'}},
                           'description': 'Открытие вклада',
                           'to': 'Счет 77613226829885488381'}) == 'Внесение наличности на счет -> Счет **8381'


def test_get_last_transaction():
    assert get_last_transaction('operations.json') == [
